...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        {
            // First part of help message is specific to SnapCoin server / RPC client
            std::string strUsage = _("SnapCoin version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  snapcoin [options]                     " + "\n" +
                  "  snapcoin [options] <command> [params]  " + _("Send command to -server or snapcoin") + "\n" +
                  "  snapcoin [options] help                " + _("List commands") + "\n" +
                  "  snapcoin [options] help <command>      " + _("Get help for a command") + "\n";
...
{
    uiInterface.ThreadSafeMessageBox(str, _("SnapCoin"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("SnapCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: snapcoin.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: snapcoin.pid)") + "\n" +
...
//    }
    ...
    // Make sure only a single snapcoin process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  SnapCoin is probably already running."), GetDataDir().string().c_str()));
    ...
    // requested to kill snapcoin-qt during the last operation. If so, exit.
